...
DECLARE @Iteration INT, @EndIteration INT, @AmplifierCode CHAR(1) = 'A';
...
WHILE (@Iteration <= @EndIteration)
	BEGIN
		...
		SELECT @AmplifierCode, @Iteration+1, 1, PhaseA FROM AmplifierChecks WHERE Iteration = @Iteration+1

		-- Just for Amplifier A
		INSERT INTO IntCodeInputs (AmplifierCode, Iteration, InputNumber, Input)
		SELECT @AmplifierCode, @Iteration+1, 2, 0

		WHILE (SELECT FinalOutput FROM AmplifierChecks WHERE Iteration = @Iteration) IS NULL
			BEGIN
				Print 'Waiting for E'
				WAITFOR DELAY '00:00:10' -- wait until Amplifier E has completed
			END
